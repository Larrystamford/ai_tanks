...
namespace Complete
{
    ...
    public class AiTankMovement : MonoBehaviour
    {
        ...
        private AiTankShooting aiShooting;
...
//            Debug.Log("player object", player);
            ...
            aiShooting = agent.GetComponent<AiTankShooting> ();
        ...
        private void Update ()
        {
            ...
            if (CanAttackPlayer() && shootingRechargeTime >= reloadTime) {
                float launchForce = 0.9f;
                Vector3 direction = player.transform.position - agent.transform.position;
                launchForce = launchForce * direction.magnitude;

                aiShooting.Fire(launchForce);
        ...
        public bool CanAttackPlayer() {
            ...
            if (direction.magnitude < shootDistance && angle < visAngle) {
                return true;
            }
            return false;
        }
